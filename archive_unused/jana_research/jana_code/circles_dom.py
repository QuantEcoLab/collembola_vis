...
if slike:
    slika_path = slike[0]  # Use only the first image
    ...
    for region in props:
        ...
        radii.append(r)
        if r > 200:
            continue  # Skip drawing circles larger than 200 pixels
        drawn_count += 1
        circ = Circle(
            (x, y), r,
            edgecolor="red",
    ...
    print(f"Total circles detected: {len(radii)}")
    print(f"Circles drawn (r ≤ 200): {drawn_count}")

    # Plot histogram of circle sizes
    if radii:
        plt.figure(figsize=(6, 4))
        plt.hist(radii, bins=20, color='blue', alpha=0.7)
        plt.xlabel('Radius (pixels)')
        plt.ylabel('Count')
        plt.title('Histogram of Circle Sizes')
        plt.tight_layout()
        plt.show()
else:
    print("No images found in 'slike' directory.")

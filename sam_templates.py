...
def load_templates(template_dir: Path) -> List[np.ndarray]:
    ...
    for ext in ("*.png", "*.jpg", "*.jpeg"):
        for path in sorted(template_dir.glob(ext)):
            try:
...
def match_templates(
    ...
    for ti, tpl in enumerate(templates):
        for sf in scales:
            ...
            tpl_norm = (tpl_resized - tpl_resized.mean()) / (tpl_resized.std() + 1e-9)
            ...
            if debug_ncc_dir is not None:
                ...
                _imsave(str(debug_ncc_dir / fname), (resp_norm * 255).astype(np.uint8))
            # Threshold
...
def main() -> None:
    ...
    for idx, (x1, y1, x2, y2, score) in enumerate(
        tqdm(candidates, desc="SAM template prompts"), start=1
    ):
    ...
    if args.debug_candidates and debug_rows:
        ...
        print(f"Wrote candidate debug CSV to {dbg_path}")
    print(
        f"Template-guided SAM detected {len(masks)} candidates (from {len(candidates)} NCC peaks)."
    )
